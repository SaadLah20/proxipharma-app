-- Désactive les alertes hors-app SMS (patient + pharmacien).
-- OTP inscription / auth = Supabase + Twilio Verify (inchangé).
-- Hors-app : in-app + e-mail + WhatsApp uniquement.

-- Préférences : plus de SMS métier
update public.notification_external_prefs
set sms_enabled = false
where sms_enabled = true;

-- File : annuler les SMS encore pending/processing
update public.notification_external_queue
set
  status = 'failed',
  attempt_count = 99,
  last_error = 'annulé : alertes SMS métier désactivées (juin 2026)',
  sent_at = null
where channel = 'sms'::public.notification_external_channel_enum
...
begin
  ...
  v_phone_channel_allowed := v_is_patient
...
comment on function public._enqueue_external_notifications_from_app_row() is
  'File externe : e-mail + WhatsApp (pas de SMS métier). WhatsApp patient selon prefs et event_type pilote.';

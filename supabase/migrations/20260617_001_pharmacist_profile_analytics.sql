-- Analytics fiche pharmacie : patient_id sur les événements + RPC agrégée (période, client, journal).

alter table public.pharmacy_engagement_events
  add column if not exists patient_id uuid references public.profiles (id) on delete set null;

comment on column public.pharmacy_engagement_events.patient_id is
  'Patient connecté à l’origine de l’événement ; null si visiteur anonyme.';

create index if not exists pharmacy_engagement_events_pharmacy_patient_created_idx
  on public.pharmacy_engagement_events (pharmacy_id, patient_id, created_at desc)
  where patient_id is not null;

create index if not exists pharmacy_engagement_events_pharmacy_anon_created_idx
  on public.pharmacy_engagement_events (pharmacy_id, created_at desc)
  where patient_id is null;

-- ---------------------------------------------------------------------------
-- Analytics : KPIs, séries, clients, journal (engagement + dossiers + promos)
-- ---------------------------------------------------------------------------
...
revoke all on function public.pharmacist_profile_analytics(timestamptz, timestamptz, uuid, text, text, int, int) from public;
grant execute on function public.pharmacist_profile_analytics(timestamptz, timestamptz, uuid, text, text, int, int) to authenticated;

comment on function public.pharmacist_profile_analytics(timestamptz, timestamptz, uuid, text, text, int, int) is
  'Analytics fiche publique : KPIs, clients touchés, journal (vues/clics/dossiers/promos) filtrable par période et patient.';

-- Retrait du statut intermédiaire dossier `processing` : après validation patient le dossier
-- reste `confirmed` jusqu’au passage `treated` via `pharmacist_mark_request_treated`.
-- La valeur enum `processing` peut subsister en base (PostgreSQL) mais n’est plus assignée.

UPDATE public.requests
SET status = 'confirmed'::public.request_status_enum, updated_at = now()
WHERE status = 'processing'::public.request_status_enum;

-- ---------------------------------------------------------------------------
-- Amendements fourniture : ne plus passer confirmed → processing
-- ---------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.pharmacist_record_supply_amendments(uuid, jsonb) IS
  'Journalise des ajustements post-validation (+ canal confirmation) ; le statut dossier reste inchangé.';

-- ---------------------------------------------------------------------------
-- Marquer traitée : uniquement depuis confirmed
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Saisie réservé / commandé / arrivé : ne plus modifier le statut dossier
-- ---------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.pharmacist_set_post_confirm_fulfillment(uuid, public.post_confirm_fulfillment_enum) IS
  'Après validation : réservé / commandé / arrivé en officine ; ne change pas le statut dossier (confirmed → treated via RPC dédiée).';

-- ---------------------------------------------------------------------------
-- Passage planifié patient
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Annulation / abandon / comptoir / clôture
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Notifications in-app (libellés sans étape « processing »)
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  n_body :=
    ...
    || CASE p_status
      ...
      WHEN 'confirmed' THEN E'Le patient a confirmé la sélection. Enregistrez réservations / commandes puis validez la préparation pour activer le comptoir.'
...
COMMENT ON FUNCTION public._emit_in_app_notifications_for_status_history() IS
  'Notifs patient + pharmacien sur journal statut ; pharmacien : exclut changed_by pour éviter l''auto-notification.';

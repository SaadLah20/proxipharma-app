-- Pricing parapharmacie : règles par marque (remplace laboratoire pour la résolution marge).
-- Table pharmacy_pricing_laboratory_rules conservée mais plus lue par les RPC.

create table if not exists public.pharmacy_pricing_brand_rules (
  ...
  unique (pharmacy_id, brand_key)
);

create index if not exists pharmacy_pricing_brand_rules_pharmacy_idx
  on public.pharmacy_pricing_brand_rules (pharmacy_id);

alter table public.pharmacy_pricing_brand_rules enable row level security;

drop policy if exists "pharmacy_pricing_brand_rules_staff" on public.pharmacy_pricing_brand_rules;
create policy "pharmacy_pricing_brand_rules_staff"
  on public.pharmacy_pricing_brand_rules for all to authenticated
  using (
    exists (
      select 1 from public.pharmacy_staff ps
      where ps.pharmacy_id = pharmacy_pricing_brand_rules.pharmacy_id and ps.user_id = auth.uid()
    )
  )
  with check (
    exists (
...
create or replace function public._normalize_brand_key(p_brand text)
returns text
language sql
immutable
as $$
  select upper(btrim(coalesce(p_brand, '')));
$$;
...
begin
  ...
  if v_margin is null then
    v_brand_key := public._normalize_brand_key(v_prod.brand);
    if v_brand_key <> '' then
      select r.margin_pct into v_margin
      from public.pharmacy_pricing_brand_rules r
      where r.pharmacy_id = p_pharmacy_id and r.brand_key = v_brand_key;
    end if;
  end if;
...
create or replace function public.pharmacist_pricing_distinct_brands()
returns table (brand_key text, brand_display text, product_count bigint)
language sql
stable
security definer
set search_path = public
as $$
  select
    public._normalize_brand_key(p.brand) as brand_key,
    max(btrim(p.brand)) as brand_display,
    count(*)::bigint as product_count
  from public.products p
  where p.is_active = true
    and p.product_type = 'parapharmacie'
    and btrim(coalesce(p.brand, '')) <> ''
  group by public._normalize_brand_key(p.brand)
  order by brand_display;
$$;
...
revoke all on function public._normalize_brand_key(text) from public;
grant execute on function public._normalize_brand_key(text) to authenticated;

revoke all on function public.pharmacist_pricing_distinct_brands() from public;
grant execute on function public.pharmacist_pricing_distinct_brands() to authenticated;

comment on table public.pharmacy_pricing_brand_rules is
  'Marge parapharmacie par marque (products.brand) ; priorité sous produit, au-dessus du global.';

...
export function lineEventTitle(kind: LineEventKind, audience: LineHistoryAudience): string {
  const ph = audience === "pharmacist";
  switch (kind) {
    case "origin_patient_request":
      return ph ? "Demande initiale du patient" : "Vous l'avez demandé";
    case "origin_pharmacist_proposed":
      return ph ? "Produit proposé par l'officine" : "Proposé par la pharmacie";
    case "pharmacist_response":
      return ph ? "Réponse publiée" : "Réponse de la pharmacie";
    ...
    case "amend_validated_qty_change":
      return ph ? "Quantité modifiée" : "La quantité a été modifiée";
    ...
    case "counter_picked_up":
      return "Retrait au comptoir enregistré";
    case "counter_unset":
      return ph ? "Remis en attente au comptoir" : "Remis en attente de passage";

...
export const patientConsultationRequestDossierSectionShellClass =
  "border-violet-200/40 bg-gradient-to-br from-violet-50/25 via-white to-fuchsia-50/10 ring-1 ring-violet-100/30";

/** Carte ligne dans un bucket (validée, traitée, archive, répondue…). */
export const patientConsultationRequestLineCardClass =
  "w-full min-w-0 rounded-lg border border-violet-200/50 bg-card px-2 py-2.5 shadow-sm ring-1 ring-violet-100/25";
...
export function patientConsultationRequestDossierHeaderShellClass(): string {
  return "rounded-xl border border-violet-200/50 bg-card shadow-sm ring-1 ring-violet-100/30";
}

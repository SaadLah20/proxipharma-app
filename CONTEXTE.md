...
- **Ordonnance (local)** : après réponse pharma = parcours demande produits ; saisie scan → **`patient_request`** (sans motif) ; compléments → **`pharmacist_proposed`** ; **Saisi ordonnance** ; scan repliable **`PrescriptionScanCollapsible`** ; thème ambre (header, hubs, validé patient).
...
**Mise à jour 2026-05-30 — refonte UX en pause (branche séparée)** :
- Branche **`design/ux-refonte-2026`** : charte Glovo-like app entière (**`lib/design-system/`**, **`docs/DESIGN-SYSTEM.md`**, **`docs/UX-REFONTE-CHECKLIST.md`**) — commits **`3b189a7`**, **`89b35d1`** ; **non mergée** ; test = preview PR Vercel.
- **Pause** : merge prod et développement courant = **`fix/validated-supply-ecart-ui-modal`**. Phrase ouverture sans tâche : **§13.35**.
...
- **Hub produits pharmacien** : thème sky, KPIs, compteur liste, regroupements indicatifs — `PharmacistProductDemandesDashboard`, `lib/pharmacist-product-hub-sections.ts`.
...
- **Supabase** : toutes migrations appliquées jusqu’à **`20260630_001`**. Phrase reprise **§13.34**.

**Mise à jour 2026-05-25 — hub compte patient (demandes produits, pharmacies, paramètres)** :
- **Hub** `/dashboard/demandes` : tableau de bord avec regroupements **indicatifs** (**À votre action**, **Chez la pharmacie**, **Archives**) — 3 aperçus + **Tout voir** → liste ; filtres liste = **statut** seulement (comme ordonnances) — `lib/patient-product-hub-sections.ts`, `patient-demandes-hub.tsx`, `patient-product-demandes-dashboard.tsx`.
- **Mes pharmacies** : `20260626_001`, `/dashboard/patient/pharmacies`, RPC annuaire enrichi — `lib/patient-pharmacy-crm.ts`.
- **Mes paramètres** : `/dashboard/patient/parametres`, suppression compte `app/api/patient/delete-account/route.ts` ; chrome compte `lib/platform-dashboard-chrome.ts`. Phrase reprise **§13.34**.

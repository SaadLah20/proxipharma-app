...
- **SQL** : **`20260515_001`** (pas de notif in-app pour **`counter_outcome:picked_up`**) ; **`20260516_001`** — **`expire_overdue_requests(interval)`** : expiration selon **`responded_at`** (défaut **30 min** pilote) + passe **`expires_at`** ; **`abandon_unconfirmed_responded_requests()`** délègue à **`expire_overdue_requests()`** ; cron **`service_role`** requis.

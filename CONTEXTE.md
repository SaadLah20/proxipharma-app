...
- **Consultations libres** : **`/pharmacie/[id]/consultation-libre`** (texte + 3 photos) ; onglets **Conversation | Produits** ; **`ConsultationDetailStickyChrome`** — récap dossier + onglets **sticky** en haut (patient + pharmacien) ; brief + messagerie inline ; publication puis validation comme demande produits ; hubs violet ; migration **`20260529_001`** ; `workflowEnabled: true`.
- **Demande produits** : notif pharmacien dédiée si patient change la date de passage — **`20260531_002`**.
- **Phrase de reprise** : **`CAHIER_DES_CHARGES.md` §13.23** (session **2026-06-01** — lot produits **`aec8fad`** + ordonnance alignée demande produits en local).

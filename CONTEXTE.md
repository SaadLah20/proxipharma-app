...
**Mise à jour 2026-05-09 — focus mobile après validation + traçabilité** (pas de nouvelle migration) :
- **`lib/build-patient-line-timeline-fr.ts`** : chronologie par ligne (origine → réponse officine → validation patient → audits `audit_v1` filtrés par produit → amendements **`request_supply_amendments`** avec **`request_item_id`** → bloc **Situation actuelle**).
- **`app/dashboard/demandes/[id]/PatientProductRequestActions.tsx`** : en **`confirmed` / `processing` / `treated`**, affichage **compact** des **lignes retenues** (`bucketPatientValidatedLinesThreeWays`), groupements **À réserver** / **À commander** / écarts si besoin ; **`<details>`** pour **lignes non retenues** (fermé par défaut) ; bouton **Historique** → modal ; date passage + contact officine + abandon inchangés.
- **`app/dashboard/demandes/[id]/page.tsx`** : passe **`requestTimelineMeta`** (dates dossier), **`dossierHistoryRows`** ; pour les statuts ci-dessus, charge **`request_status_history`** dans le **`Promise.all`** du détail (limite 30) pour alimenter la timeline **sans** cliquer « Rafraîchir ».
- **Lots hubs / répondue antérieurs (même période)** : cartes liste compactes (**`demande-hub-ui.tsx`**) + **`updated_at`** dans les `select` des hubs ; **`lib/request-display.ts`** **`responded`** → **Répondue - à valider** ; **`respondedFrozenView`** sans contrôle **Réagir** hors mode édition.

...
- **Hub** `/dashboard/demandes` : tableau de bord par regroupements (**À votre action**, **Chez la pharmacie**, **Archives**) — 3 aperçus + **Tout voir** ; liste avec filtres **statut** + **regroupement** cumulables — `lib/patient-product-hub-sections.ts`, `patient-demandes-hub.tsx`, `patient-product-demandes-dashboard.tsx`.

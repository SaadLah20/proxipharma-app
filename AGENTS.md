...
**UI patient demande produits (mai 2026, branche `fix/validated-supply-ecart-ui-modal`)** — roadmap §4.6 : **envoyée** + **répondue** affinées (session **2026-05-25**, phrase **§13.32**). **Saisie publique** : `components/pharmacy/patient-demande-produits-ui.tsx` (`ProductRequestLinePanel`, explorer catalogue sky). **Envoyée / répondue** : en-tête **`PatientProductRequestDossierHeader`** (sky) ; lignes envoyée **`PatientProductRequestCompactLine`** ; **répondue** = **`RespondedPatientLineChooser`** (`patient-responded-line-chooser.tsx`) : case sur vignette, badges Ta demande / Alternative / Ajout Officine, motif dans le bloc, onglets si alternatives, grisé si indispo, barré si non retenu, modale confirmation sky. **Validée / traitée / terminés** : cartes compactes groupées, **`<details>`** lignes non retenues, **Historique** + **`request_status_history`**, **`PatientLineSuiviStrip`**, **`PatientRequestOutcomeBanner`** — voir journal §10 sessions antérieures. Hub : cartes + **`updated_at`**. Libellé **`responded`** · **« Répondue - à valider »**. Badge **« Ajout officine »** post-validé : **`lib/supply-line-post-confirm.ts`**.

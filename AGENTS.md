...
**Fiche digitale pharmacie (mai 2026)** — **`CAHIER_DES_CHARGES.md` §10 (session 2026-05-22)** + **§13.28**. Migrations **`20260606_001`** · **`20260607_001`** · **`20260608_001`** (avis 1–5, sans commentaire au pilote). Publique **`/pharmacie/[id]`** : onglets grille 4 col., Infos (`pharmacy-profile-contact-grid.tsx`), Horaires, **`PharmacyRatingForm`** (note compacte après vote). Pharmacien **`ma-fiche`** : upload **couverture** / **logo** → chemins **`pharmacies/{id}/cover-{ms}.webp`** | `logo-{ms}.webp` (**`lib/pharmacy-media.ts`**, spec **`lib/pharmacy-cover-spec.ts`** 21:9) · **`horaires-garde`**. Storage : **`20260524_001`**.

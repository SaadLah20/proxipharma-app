...
**Types de demande (registre)** — **`lib/request-kinds/`** : config par type (`product_request`, `prescription`, `free_consultation`) — thème header, routes hubs, capacités, refs **D/O/C**. Workflow commun → **`lib/request-kinds/shared-capabilities.ts`** + **`lib/demandes-hub-buckets.ts`**. **Ordonnances** : **`lib/prescription-media.ts`**, **`components/requests/prescription/`** (scan, modal qté prescrite/dispo, badge Ordonnance) ; migrations **`20260525_*`**, **`20260526_001`**. **Consultation libre** : **`lib/consultation-media.ts`**, **`components/requests/consultation/consultation-brief-panel.tsx`**, **`/pharmacie/[id]/consultation-libre`** ; migration **`20260529_001`**. Synthèse retours : **`docs/workflow-ordonnance-consultation-REPONSES.md`**. Phrase de reprise : **`CAHIER_DES_CHARGES.md` §13.20.

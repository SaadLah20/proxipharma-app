...
**Types de demande (registre)** — **`lib/request-kinds/`** : config par type (`product_request`, `prescription`, `free_consultation`) — thème header, routes hubs, capacités, refs **D/O/C**. Workflow commun → **`lib/request-kinds/shared-capabilities.ts`** + **`lib/demandes-hub-buckets.ts`**. **Ordonnances (2026-06-01)** : après réponse pharma = **même parcours** que demande produits ; lignes scan en **`line_source: patient_request`** (libellé **Saisi ordonnance**, pas motif) — legacy `pharmacist_proposed` + motif « Saisie depuis ordonnance » reconnu ; compléments hors scan = **`pharmacist_proposed`** + motif. **`lib/prescription-pharmacist-lines.ts`**, **`lib/prescription-patient-labels.ts`**, **`lib/prescription-ordonnance-line-qty.ts`**, **`components/requests/prescription/`** (dont **`prescription-scan-collapsible.tsx`**) ; thème **ambre** (`prescription.config.ts` **`headerShellForStatus`**, hubs **`demande-hub-ui.tsx`**). Migrations **`20260525_*`**, **`20260526_001`**, **`20260530_001`**, **`20260531_001`**. **Consultation libre** : **`components/requests/consultation/`** (brief, **`consultation-detail-sticky-chrome.tsx`**) ; migration **`20260529_001`**. **Demande produits** : **`20260531_002`** ; notifs UI **`20260601_001`** (traitée, amendements, passage, réception officine). Synthèse retours : **`docs/workflow-ordonnance-consultation-REPONSES.md`**. Phrase de reprise : **`CAHIER_DES_CHARGES.md` §13.23**.

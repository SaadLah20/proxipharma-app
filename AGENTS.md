...
**Fiche digitale pharmacie (mai 2026)** — **`CAHIER_DES_CHARGES.md` §10 (session 2026-05-19 suite)** + **§13.27**. Migrations **`20260606_001`** (profil, services, horaires, gardes) · **`20260607_001`** (notes ligne figées). Publique **`/pharmacie/[id]`** (`components/pharmacy/pharmacy-public-profile.tsx`). Pharmacien **`/dashboard/pharmacien/ma-fiche`** (upload **couverture** / **logo** : **`lib/pharmacy-media.ts`**, bucket **`public-assets`**, chemins `pharmacies/{id}/cover.webp` | `logo.webp`) · **`horaires-garde`**. Storage pharmacien : policies **`20260524_001`**.

**Ne pas réécrire tout le détail produit** sans relire **`CONTEXTE.md` §6** et **`CAHIER_DES_CHARGES.md` §4.4–§4.5 + dernier §10 Journal + §13.27** (phrase de reprise ; §13.12 / **`20260509_*`** pour supply post-validé). Pour une **ouverture sans tâche** : **`CAHIER_DES_CHARGES.md` §13.11**.

...
/** Monté avec key produit/rapport — chargement initial sans setState synchrone dans useEffect. */
function CatalogProductReportForm({
  productId,
  productName,
  reportId,
  onClose,
...
export function CatalogProductReportModal({
  ...
  return (
    <AppModalOverlay
      open
      aria-labelledby="catalog-report-modal-title"
      className="p-0 sm:p-4"
      onBackdropClick={onClose}
    >
      <CatalogProductReportForm
        key={`${productId}-${reportId ?? "new"}`}
        productId={productId}

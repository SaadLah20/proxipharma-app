      ...
      <section className={clsx("rounded-xl border-2 bg-white p-2 shadow-sm", border, className)}>
        ...
        {hasTwo ? (
          <PageTabs
            ...
            className={clsx("mt-2 flex gap-1", comfort && "md:hidden")}
          />
        ) : null}
        <div
          className={clsx(
        ...
        >
          {(paths.page1 && (!hasTwo || activeTab === 1)) || !hasTwo ? (
            <PrescriptionImageSlot label="Page 1" url={urls.page1} comfort={comfort} onExpand={openLightbox} />
          ) : null}
          {hasTwo && paths.page2 ? (
            <div className={clsx(activeTab === 2 ? "block" : "hidden md:block")}>
              <PrescriptionImageSlot label="Page 2" url={urls.page2} comfort={comfort} onExpand={openLightbox} />
            </div>
          ) : null}
        </div>
        {comfort ? (
          <p className="mt-2 hidden text-[10px] text-muted-foreground md:block">
            Sur ordinateur : cliquez sur une page pour l’agrandir et zoomer.
          </p>
        ) : null}
      </section>
      {lightbox ? (
        <PrescriptionLightbox label={lightbox.label} url={lightbox.url} accent={accent} onClose={() => setLightbox(null)} />
      ) : null}
    </>
  );
}
...
function PrescriptionImageSlot({
  ...
  const canExpand = comfort && Boolean(url) && !loadErr;
    ...
    <div className="overflow-hidden rounded-lg border border-border/70 bg-muted/20">
      ...
      {url && !loadErr ? (
        <button
          type="button"
          disabled={!canExpand}
          onClick={() => (canExpand ? onExpand(label, url) : undefined)}
          className={clsx("block w-full bg-white", canExpand && "cursor-zoom-in")}
        >
